# Acting Scene Background â€” Implementation Guide

**Purpose:** Port the NexusQ cinematic acting-scene animation to another website (e.g. AuditionQ).  
**Source project:** NexusQ (`cinematic-background-test` branch)  
**Date:** 2026-08-11  
**Status:** Test-proven on NexusQ; ready to copy

---

## 1. What this animation is

A full-viewport animated SVG background with:

- An **actor** performing in front of the camera
- A **cameraman** wearing a **director cap**, operating a cinema camera
- Studio **scenery** (cyclorama, lights, director chair, boom mic, cases, clapperboard)
- Smooth Framer Motion loops
- **Light mode + dark mode** support
- Mobile-reduced motion + `prefers-reduced-motion` support

It sits **behind** all website content and does not block clicks.

---

## 2. Files you need to create

| Action | Path on target site | Notes |
|--------|---------------------|--------|
| **CREATE** | `components/ActingSceneBackground.tsx` (or `app/components/...`) | Copy full source from this guide (Section 9) or from `docs/ActingSceneBackground.tsx` |
| **MODIFY** | Homepage / layout page (e.g. `app/page.tsx` or `app/layout.tsx`) | Mount background + wrap content (Section 5) |
| **OPTIONAL** | Theme / CSS tokens | Only if site has no Tailwind `dark:` or no `bg-nq-bg` â€” retune veil (Section 6) |

### Files you do NOT need

- `AuroraBackground.tsx`
- `FilmSetBackground.tsx`
- `public/film-background.jpg`
- NexusQ card / section CSS fixes (unless the target site has the same transparency bugs)
...
- React 18+ (or 19)
- Client Components if using Next.js App Router (`"use client"`)
- Tailwind CSS recommended (component uses Tailwind utility classes + `dark:`)

Check `package.json` for `"framer-motion"`. If present, skip install.

---

## 4. Architecture (must keep)

```
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚  ActingSceneBackground              â”‚
â”‚  fixed Â· inset-0 Â· z-0              â”‚
â”‚  pointer-events-none                â”‚
â”‚  full-bleed SVG (slice cover)       â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
                  â–²
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚  Site content (z-10)                â”‚
â”‚  Navbar Â· Hero Â· Sections Â· Footer  â”‚
â”‚  stationary â€” never animated        â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
```

Critical rules:

1. Background: `pointer-events-none` + `fixed inset-0` + `z-0`
2. Content: `relative z-10`
3. Parent: `relative` + `overflow-x-hidden`
4. SVG: `preserveAspectRatio="xMidYMid slice"` + `h-full w-full` so it fills the screen

---

## 5. Exact page wiring (what to change)
...
// Keep your old background import commented â€” do not delete the file

export default function Home() {
  return (
    <main className="relative min-h-screen overflow-x-hidden">
      {/* Optional: comment out old background for the test */}
      {/* <OldBackground /> */}
...
### NexusQ reference wiring (`app/page.tsx`)
...
## 6. Theme / readability adaptations

### If the target site uses Tailwind dark mode

The component already has `dark:` variants. Ensure `<html class="dark">` (or your theme provider) works.

### If there is no `bg-nq-bg`

In `ActingSceneBackground.tsx`, find the readability veil near the bottom:

```tsx
<div className="absolute inset-0 bg-nq-bg/30 dark:bg-nq-bg/38" />
```

Replace with site tokens, for example:
...
Tune until hero text is readable.

### Cards / sections over the background

Use **solid** card backgrounds. Avoid Tailwind opacity modifiers on CSS-variable colors (e.g. `bg-nq-surface/85`) â€” they often fail and cards look invisible.
...
1. Create a **test branch** on the target repo (do not ship on main first)
2. Install `framer-motion` if needed
3. Create `ActingSceneBackground.tsx` with the full source (Section 9)
4. Wire homepage with fixed background + `z-10` content wrapper
5. Comment out / disable old background **without deleting** it
6. Test: desktop, mobile, light mode, dark mode
7. Test: `prefers-reduced-motion` (OS reduce motion)
8. Tune veil opacity for text contrast
9. Confirm no click blocking (buttons, links, forms)
10. Confirm no horizontal scroll
11. Soft-launch behind a feature flag if available
12. Keep one-line revert path (swap back to old background)

---

## 8. Performance & safety notes

- Prefer transforms/opacity only (already done)
- Mobile uses smaller motion amplitude (`amp = 0.5`)
- Respect reduced motion
- Start on homepage only â€” avoid mounting on every heavy dashboard page
- Optional later: pause when `document.visibilityState === "hidden"`
- Do **not** rewrite to Canvas/WebGL for v1

---

## 9. Full source code â€” `ActingSceneBackground.tsx`

Copy this entire file to the target project.

```tsx
```

## 9. Full source code — ActingSceneBackground.tsx
...
/**
 * TEST ONLY â€” cinematic acting-scene background.
...
export default function ActingSceneBackground() {
  ...
  return (
    ...
    >
      ...
      {/* Full-viewport scene â€” cover entire screen (not a bottom box) */}
      <div className="absolute inset-0">
        ...
        >
          ...
          {/* â•â•â•â•â•â•â•â•â•â•â•â• SCENERY (behind characters) â•â•â•â•â•â•â•â•â•â•â•â• */}
          ...
          {/* Left scenery â€” director chair + plant */}
          ...
          {/* Right scenery â€” equipment cases */}
          <g filter="url(#softShadow)">
          ...
          {/* â•â•â•â•â•â•â•â•â•â•â•â• ACTOR (connected silhouette) â•â•â•â•â•â•â•â•â•â•â•â• */}
          <motion.g
          ...
          >
            ...
            {/* Left arm â€” attached at shoulder, one continuous limb */}
            <motion.g
            ...
            {/* Right arm â€” raised acting gesture, attached at shoulder */}
            <motion.g
          ...
          {/* â•â•â•â•â•â•â•â•â•â•â•â• CAMERA + TRIPOD â•â•â•â•â•â•â•â•â•â•â•â• */}
          <g filter="url(#softShadow)">
            <path
          ...
          {/* â•â•â•â•â•â•â•â•â•â•â•â• CAMERAMAN (connected, director look) â•â•â•â•â•â•â•â•â•â•â•â• */}
          <motion.g
          ...
          >
            ...
            {/* Torso / director jacket â€” one piece into hips */}
            <path
              d="M922 295
            ...
            />
            {/* Red director scarf â€” attached to collar */}
            <path
              d="M952 308
...
From the NexusQ repo (or this docs folder):

```bash
# Copy component into another Next.js app
cp docs/ActingSceneBackground.tsx /path/to/other-site/app/components/ActingSceneBackground.tsx
...
1. Comment out `<ActingSceneBackground />`
2. Re-enable the previous background component
3. Keep `ActingSceneBackground.tsx` in the repo until you decide permanently

---

## 12. Source of truth

- NexusQ branch: `cinematic-background-test`
- Component path: `app/components/ActingSceneBackground.tsx`
- This guide + standalone copy: `docs/` folder
- Do not merge to NexusQ `main` until explicitly approved

---

*Generated for porting the NexusQ acting-scene background test to another website.*
